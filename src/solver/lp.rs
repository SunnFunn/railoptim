...
/// Штраф за 1 вагон неудовлетворённого спроса / неиспользованного предложения (руб.).
///
/// Должен быть заведомо выше стоимости любого реального плеча (тариф + порожний ход
/// + промывка) — тогда LP/MIP всегда предпочтёт реальную дугу штрафу. При слишком
/// низком значении решатель начинает «экономить» на распределении вагонов, оставляя
/// их в dummy-узлах: формально это снижает `obj`, но бизнес-смысла в этом нет.
///
/// Историческое значение 400 000 руб./ваг. на реальных данных оказалось
/// недостаточным — MIP предпочитал dummy части дорогих плеч. Поднято до 1 000 000
/// руб./ваг. (покрывает все реальные сочетания тариф+порожняк+промывка в данных).
///
/// Верхняя граница — вменяемость: PENALTY ≫ max(arc.cost) означает, что любое
/// улучшение покрытия на 1 вагон всегда доминирует над изменением real_cost,
/// что и требуется.
