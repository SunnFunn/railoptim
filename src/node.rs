...
pub struct DemandNode {
    ...
    /// Спрос АПИ: ГУ-12 его не уменьшает. Потолок заявок для вагонов с дорог РФ —
    /// [`Self::gu12_cap`].
    pub car_count: i32,
    /// Потолок ГУ-12 (правило 3) на подсыл **с российских дорог** в этот узел погрузки.
    ///
    /// `None` — ограничения нет (проверка выключена, узел промывки или погрузка
    /// на инотерритории). `Some(n)` — сумма потока по дугам с `gu12_exempt = false`
    /// не больше `n` (в том числе 0: российской дуги в узел нет, узел остаётся
    /// для вагонов с инотерриторий). Вагоны, образовавшиеся на инотерритории,
    /// этот потолок не расходуют.
    pub gu12_cap: Option<i32>,
    /// Количество вагонов на станции для оценки ее загруженности.
    pub cars_on_station: i32,
}

impl DemandNode {
    /// Потолок ГУ-12 строже спроса АПИ: в модели нужен отдельный ряд на дуги
    /// с российских дорог. `None` — ряд не нужен.
    pub fn gu12_russian_limit(&self) -> Option<i32> {
        let cap = self.gu12_cap?;
        if self.purpose != DemandPurpose::Load || cap >= self.car_count {
            None
        } else {
            Some(cap.max(0))
        }
