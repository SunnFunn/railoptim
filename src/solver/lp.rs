...
/// `150 000 − надбавка промывочного маршрута (правило 6, по умолчанию 50 000) = 100 000`
/// руб. Типовое плечо с учётом промывки и последующего подсыла чистым — 70–100 тыс.;
/// более дальняя промывка (через всю страну) проигрывает остатку, и вагон идёт в отстой.
///
...
/// `MaxEmptyRunDistanceKm` и правило 6 (`DirtySameCargoMaxCostRatioToWash`) в
/// `classify_pair`; там же поощрение `DirtySameCargoRewardShare` решает спор грязного
/// и чистого вагона за одну заявку.
///
