...
fn write_output_sheet(workbook: &mut Workbook, records: &[OutputRecord]) -> Result<(), XlsxError> {
    ...
    // `r.cost` — уже чистый тариф из АПИ (`TaskArc::tariff_cost` для назначений
    // оптимизации; тарифы отстоя / путей / ремонта / факта берутся из АПИ как есть).
    // Раньше здесь обратным расчётом вычитался штраф за срок — теперь модельные
    // надбавки в OutputRecord не попадают вовсе, пересчёт не нужен.
    for (row_idx, r) in records.iter().enumerate() {
        ...
        ws.write_with_format(row, 25, r.cost,                             &dec)?;
