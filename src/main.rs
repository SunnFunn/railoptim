...
async fn main() -> Result<()> {
    ...
    // Правило 5, шаг 1: пробное чтение conv-redis (`telegrams_db`). Солвер ещё не
    // использует снимок; нет пароля/Redis — предупреждение, оптимизация идёт дальше.
    data::probe_conventions_at_startup();
